...
const char * const BITCOIN_CONF_FILENAME = "easynodecoin.conf";
const char * const BITCOIN_PID_FILENAME = "easynodecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("easynodecoin"))) {
...
#else
    const char* pszModule = "easynodecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EasynodecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EasynodecoinCore
    // Mac: ~/Library/Application Support/EasynodecoinCore
    // Unix: ~/.easynodecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EasynodecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EasynodecoinCore";
#else
    // Unix
    return pathRet / ".easynodecoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty easynodecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override easynodecoin.conf
...
void AllocateFileRange(FILE *file, unsigned int offset, unsigned int length) {
    ...
    SetFilePointerEx(hFile, nFileSize, 0, FILE_BEENC);
    SetEndOfFile(hFile);
